...
\begin{center}
  {\Large\textbf{\underline{FULL CORPORATE OFFER FOR }}}
\end{center}

\vspace{0.3cm}

\
oindent\textbf{Date: Mock fco date} \hfill \textbf{Mock reference no}
...
\
oindent\textbf{Company Name:} Mock buyer company\\
\textbf{Address:} Mock buyer address\\
\textbf{Website:} Mock buyer website\\
\textbf{Email:} Mock buyer email\\
\textbf{Phone Number:} Mock buyer phone
...
\
oindent We, \textbf{}, hereby formally confirm our intent and capability to supply \textbf{}. We affirm that our company is fully prepared, willing, and able to sell the product strictly in accordance with the specifications, quantities, pricing, and commercial terms outlined in this \textbf{Full Corporate Offer (FCO).} This declaration is issued with full corporate authority and complete responsibility on behalf of our company.
...
Trial Container & Mock trial container \\ Price & Mock trial price \\ Trial Shipment Value & Mock trial shipment value \\ Trial Quantity in MT & Mock trial quantity mt \\ Loading Capacity & Mock loading capacity \\ Loading Port & Mock loading port \\ Destination Port & Mock destination port \\ Delivery Time & Mock delivery time \\ \end{tabularx}
\end{center}
...
\begin{center}
\begin{tabularx}{\textwidth}{>{\columncolor{lightgray}\bfseries\raggedright\arraybackslash}p{5.5cm} >{\raggedright\arraybackslash}X}
Quantity & Mock contract quantity \\ Price & Mock contract price \\ Monthly Shipment Value & Mock monthly shipment value \\ Annual Contract Value & Mock annual contract value \\ Contract Duration & Mock contract duration \\ Loading Capacity & Mock contract loading capacity \\ Annual Contract Quantity & Mock annual contract quantity \\ Loading Port & Mock contract loading port \\ Destination Port & Mock contract destination port \\ Delivery Time & Mock contract delivery time \\ \end{tabularx}
\end{center}
...
\multicolumn{1}{>{\columncolor{lightgray}\bfseries\raggedright\arraybackslash}p{5cm}}{CATEGORY} & \multicolumn{1}{>{\bfseries\raggedright\arraybackslash}X}{SPECIFICATION} \\ Product Name &  \\ Grade & Mock spec grade \\ Origin & Mock spec origin \\ Origin / Certification & Mock spec certification \\ Processing & Mock spec processing \\ Appearance & Mock spec appearance \\ Odor & Mock spec odor \\ Bones & Mock spec bones \\ Blood / Blood Stains & Mock spec blood stains \\ Moisture Content & Mock spec moisture \\ Damage & Mock spec damage \\ Chicken Paw Pad &  \\ Weight per Piece & Mock spec weight per piece \\ Length per Piece & Mock spec length per piece \\ Packaging & Mock spec packaging \\ Inner Packing & Mock spec inner packing \\ Carton Weight & Mock spec carton weight \\ Freezing Temperature & Mock spec freezing temp \\ Storage Temperature & Mock spec storage temp \\ Transportation Temperature & Mock spec transport temp \\ Freezing Process & Mock spec freezing process \\ Shelf Life & Mock spec shelf life \\ Packaging & Mock spec packaging note \\ \end{tabularx}
\end{center}
...
Company Name & Mock buyer company \\ Address & Mock buyer address \\ Contact Person & Mock buyer contact person \\ Email Address & Mock buyer email \\ Country & Mock buyer country \\ Phone No. & Mock buyer phone \\ \end{tabularx}
\end{center}
...
\begin{center}
\begin{tabularx}{\textwidth}{>{\columncolor{lightgray}\bfseries\raggedright\arraybackslash}p{4cm} >{\raggedright\arraybackslash}X}
Bank Name & Mock buyer bank name \\ Bank Address & Mock buyer bank address \\ Bank Swift Code & Mock buyer bank swift \\ Account Name & Mock buyer account name \\ Account Number & Mock buyer account number \\ Bank Officer/Email & Mock buyer bank officer email \\ \end{tabularx}
\end{center}
...
\
oindent\textbf{Please Note:} A final SPA will be signed between end seller and \textbf{Mock buyer company} once this FCO is accepted, signed and sealed by the buyer.

\vspace{0.4cm}

\
oindent\textbf{VALIDITY OF OFFER}\\
This FCO remains valid until \textbf{Mock validity}
...
\
oindent\textbf{\underline{FOR AND ON BEHALF OF  (SELLER)}}
...
\textbf{Date: Mock signing date}

\vspace{2.5cm}

\
oindent\textbf{\underline{FOR AND ON BEHALF OF Mock buyer company (BUYER)}}
